...
number of anagrams for each word listed below:
    ...
    * tesla -> 10 anagrams
    * spear -> 12 anagrams
This code is one of the series to find anagrams codes. And Using dict looks like:
{length of word : {the first two alphabets : [words] } to create dictionary.
Conclusion: This code is fast but not the fastest in series. If use set to instead of list will be faster.
"""
...
def find_anagrams(s):
    ...
    return find_anagrams_helper(s, len(s), '', [])


def find_anagrams_helper(s, len_s, result, lst):
    """
    ...
    if len(result) == len_s:
        ...
        for i in range(len(s)):
            ...
            # explore:
            if len(result) == 1:
                find_anagrams_helper(s, len_s, result, lst)
