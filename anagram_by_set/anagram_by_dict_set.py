...
def main():
    ...
    while True:
        ...
        # Exclude the illegal input string - if input string is not all alphabets.
            ...
            # If there's no word found in set_ from function find_anagram().
            if len(set_) == 0:
                print(f'0  anagrams :[]')
...
def find_anagrams(s):
    ...
    return find_anagrams_helper(s, '', set(), set())


def find_anagrams_helper(s, result, set_, pass_):
    """
    ...
    :param pass_: set - the set store impossible leading characters(pre-fix) from dictionary to reduce recursive time.
        ...
        for ch in s:
            ...
            if result.count(ch) == s.count(ch) or result in pass_:
                ...
                if len(result) > 1:         # One character doesn't need to check.
                    ...
                    if not has_prefix(result, len(s)):
                        pass_.add(result)

                # explore
                find_anagrams_helper(s, result, set_, pass_)
                # un-choose
